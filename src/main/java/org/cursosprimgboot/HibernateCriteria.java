...
public class HibernateCriteria {
    public static void main(String[] args) {
        //Criteria Builder se usa principalmente para las consultas a base de datos
        //y nos da cierto control orientado a objetos sobre las consultas
        ...
        //busca un parametro entre un valor a otro
        query = criteria.createQuery(Cliente.class);
        from = query.from(Cliente.class);

        query.select(from).where(criteria.between(from.get("id"), 2L, 6L));
        //get from id entre 2 y 6
        ...
        queryString.select(criteria.upper(criteria.concat(criteria.concat(from.get("nombre")," "),
                from.get("apellido"))));
        nombres = em.createQuery(queryString).getResultList();
        nombres.forEach(System.out::println);
        ...
        from = queryObject.from(Cliente.class);
        queryObject.multiselect(from.get("id"), from.get("nombre"), from.get("apellido")).where(criteria.like
                (from.get("nombre"), "%lu%"));
        ...
        , criteria.sum(from.get("id")), criteria.max(from.get("id")), criteria.min(from.get("id")));
